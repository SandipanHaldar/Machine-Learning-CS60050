...
def gradient_descent(train_x,train_y,lamda,alpha=0.05,iterations=None):
    ...
    if iterations != None:
        for it in range(iterations):
            #updation acc to ridge egularization 
            for i in range(num_features):
                weights[i] =weights[i]*(1-(alpha*lamda)/(4*num_points))- (alpha/num_points)*np.sum(np.multiply((predictions-train_y).reshape(num_points,1),train_x[:,i].reshape(num_points,1)))
            predictions = hypothesis(weights,train_x)
            error = mean_squared_error(predictions,train_y)+(lamda/8)*(weights**2).mean()
   ...
   # print(X_train.shape[0])
    for lamda in range(1,5): 
        ...
        for i in range(1,3):
            ...
            error = mean_squared_error(predictions,test_y)+(lamda/8)*(weights**2).mean()
            print(' Error on test data = '+str(error))
    ...
    plt.title('Training error plot after ridge regularization on the deg1 curve')
    plt.xlabel('(lamda*4)')
    ...
    plt.title('Training error plot after ridge regularization on the deg8 curve')
    plt.xlabel('(lamda*4)')
    plt.ylabel('Training Error')
    lamda = list(range(1,5))
    ...
    plt.title('test error plot after ridge regularization on the deg1 curve')
    plt.xlabel('(lamda*4)')
    ...
    plt.title('Test error plot after ridge regularization on the deg8 curve')
    plt.xlabel('(lamda*4)')
    ...
    plt.title('Test error ans training error plot after ridge regularization on the deg8 curve')
    plt.xlabel('(lamda*4)')
    plt.ylabel('error')
    lamda = list(range(1,5))
    ...
    plt.title('Test error ans training error plot after ridge regularization on the deg1 curve')
    plt.xlabel('(lamda*4)')
    plt.ylabel('error')
    lamda = list(range(1,5))
