...
def main():
    ...
    clf = MLPClassifier(hidden_layer_sizes=(64,32), activation='relu', solver='sgd', alpha=.01,batch_size=32,max_iter=200)
    clf.fit(X_train,y_train)
    #print(clf.n_iter_)
    y_pred=clf.predict(X_train)
    print('Train accuracy(2B) =  '+str(accuracy_score(y_train,y_pred)*100))
    y_pred=clf.predict(X_test)
    print('Test accuracy(2B) =  '+str(accuracy_score(y_test,y_pred)*100))
